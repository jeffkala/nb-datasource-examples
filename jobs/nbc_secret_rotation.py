...
from nautobot.extras.jobs import ChoiceVar, Job, ScriptVariable, StringVar


def fetch_data_from_api(url, api_token, org_id):
    """Make the API call to fetch data from the NBC API."""
    headers = {"accept": "application/json", "content-type": "application/json", "Authorization": f"Token {api_token}"}
...
class CloudSecretRotationv1(Job):
    ...
    def run(self, *args, **data):  # pylint: disable=too-many-branches
        ...
        request_data = {
            "secret_value": {f"{data['nautobot_cloud_secret']}": f"{data['new_secret_value']}"},
